...
void FAssetTypeActions_AkAudioEvent::OpenAssetEditor( const TArray<UObject*>& InObjects, TSharedPtr<IToolkitHost> EditWithinLevelEditor )
{
	...
	FWaapiPlayerModule* WaapiPlayerModule = FModuleManager::GetModulePtr<FWaapiPlayerModule>("WaapiPlayer");
	if (WaapiPlayerModule)
	{
		UAkAudioEvent* SelectedEvent = Cast<UAkAudioEvent>(InObjects[0]);
		if (SelectedEvent)
		{
			WaapiPlayerModule->CreateAssetEditor(EToolkitMode::Type::Standalone, EditWithinLevelEditor, SelectedEvent);
			if (WaapiPlayerModule->GetAssetEditor().IsValid())
			{
				WaapiPlayerModule->GetAssetEditor()->OnEventSelected.Broadcast(SelectedEvent->GetName());
			}
		}
		else
		{
			WaapiPlayerModule->CreateAssetEditor(EToolkitMode::Type::Standalone, EditWithinLevelEditor, UAkAudioEvent::StaticClass()->GetDefaultObject());
		}
	}
}
